...
namespace GameLogic
{
    public class WordParser
    {
        ...
        public List<Word> ParseWords()
        {
            ...
            for (var col = 0; col < _grid[row].Count; col++)
                if (!IsNonCharacter(_grid[row][col]))
                {
                    ...
                    if (horizontalWord != null) words.Add(horizontalWord);

                    // Check vertically
                    var verticalWord = ExtractWord(row, col, false);
                    if (verticalWord != null) words.Add(verticalWord);
                }
        ...
        private Word ExtractWord(int row, int col, bool isHorizontal)
        {
            ...
            while (true)
            {
                var currentRow = isHorizontal ? row : row + i;
                var currentCol = isHorizontal ? col + i : col;

                // Break if out of bounds
                if (currentRow >= _grid.Count || currentCol >= _grid[0].Count) break;

                var letter = _grid[currentRow][currentCol];
                if (IsNonCharacter(letter)) break;
            ...
            if (word.Length > 1) return new Word(word.ToString(), coordinates);
